...
export async function POST(request: NextRequest) {
  try {
    ...
    const uploadUrl = await convex.mutation(
      api.fileOperations.generateUploadUrl
    );
    ...
    const transcription = await openai.audio.transcriptions.create({
      ...
      prompt:
        "This is a natural conversation. Please transcribe exactly as spoken, including all filler words like 'um', 'uh', 'like', 'you know', pauses, repetitions, and natural speech patterns. Do not clean up or edit the speech. Keep it authentic to how the person actually talks.",
    });
    ...
    const transcriptionUploadUrl = await convex.mutation(
      api.fileOperations.generateUploadUrl
    );
    ...
    const { storageId: transcriptionStorageId } =
      await transcriptionUploadResponse.json();
    console.log(
      "Transcription uploaded to Convex with storage ID:",
